...
def test_enrich_event_batch_missing_provider_uses_settings(monkeypatch):
    monkeypatch.setattr(
    ...
    assert row["xai_analysis"]["provider"] == "gemini"
    assert row["xai_analysis"]["model"] == "gemini-2.5-flash"
